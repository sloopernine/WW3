...
public class FirebaseTest : MonoBehaviour
{
    void Start()
    {
        FirebaseApp.CheckAndFixDependenciesAsync().ContinueWithOnMainThread(task =>
        {
            ...
            //StartCoroutine(RegUser("robin@omnick.se", "test123!"));
            StartCoroutine(SignIn("robin@omnick.se", "test123!"));
        });
    }

    private IEnumerator RegUser(string email, string username)
    {
        Debug.Log("Starting Registration");
        var auth = FirebaseAuth.DefaultInstance;
        var regTask = auth.CreateUserWithEmailAndPasswordAsync(email, username);
